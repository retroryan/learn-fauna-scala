...
object SpellExample  {
  ...
  private def runSpellExamples(DB_NAME: String, client: FaunaClient): Unit = {
    ...
    */
    val e1 = Element("water", "caspian sea water")
    val e2 = Element("ice", "mont shasta ice")
    val newSpell = Spell("Water Dragon's Claw", Some(Seq(e1,e2)), Option(25))
    ...
    */
    val spellOne = Spell("Chill Touch", Some(Seq(e1,e2)), Option(18))
    val spellTwo = Spell("Dancing Lights", Some(Seq(e1,e2)), Option(45))
    val spellThree = Spell("Fire Bolt", Some(Seq(e1,e2)), Option(32))
    ...
    println(s"Created list of spells from scala list: \n $spellsListSave \n")
    ...
    val allSpellsCollection = findAllSpells.collect(Field.to[Spell]).get
  ...
  case class Element(name: String, description: String)

  case class Spell(name: String, elements: Option[Seq[Element]], cost: Option[Int])

  object Element {
    implicit lazy val elementCodec: Codec[Element] = Codec.caseClass
  }

  object Spell {
    implicit lazy val spellCodec: Codec[Spell] = Codec.caseClass
  }
